...
Huffman_tree::Huffman_tree(std::shared_ptr<TreeNode<std::pair<std::string, int>>> const& lhs, 
	...
	: Binary_tree{}
	{
		...
		m_root->m_value = {"",m_root->m_lhs->m_value.second + m_root->m_rhs->m_value.second}; // sets root value to "" and the sum of both childs int values
	}

void Huffman_tree::print()const{ // prints the tree using the binary trees post post order traversion and a lambda
...
std::string Huffman_tree::generate_coding()const
{
	coding code{};
	traverse_post_order(code);
